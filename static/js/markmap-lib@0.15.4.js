...
(function (global, factory) {
  ...
  const name$3 = 'katex';
  const preloadScripts$1 = [`KaTeX/${"0.16.8"}/katex.min.js`].map(path => buildJSItem(path));
  const webfontloader = buildJSItem(`webfont/${"1.6.28"}/webfontloader.js`);
  webfontloader.data.defer = true;
  const styles$1 = [`KaTeX/${"0.16.8"}/katex.min.css`].map(path => buildCSSItem(path));
  const config$1 = {
    versions: {
      katex: "0.16.8" ,
  ...
  const name = 'hljs';
  const preloadScripts = [`highlight.js/${"11.8.0"}/highlight.min.js`].map(path => buildJSItem(path));
  const styles = [`highlight.js/${"11.8.0"}/styles/default.min.css`
        ...
        if (token.type.endsWith('_open')) {
          ...
          const revoke = this.hooks.htmltag.tap(ctx => {
            ...
            if ((data == null ? void 0 : data[0]) === 'fold') {
              current.payload = _extends$1({}, current.payload, {
